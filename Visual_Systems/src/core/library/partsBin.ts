...
export function compileThemeId(parts: { accentIdx: number, textureIdx: number, behaviorIdx: number }): string {
  return `THEME_${parts.accentIdx}${parts.textureIdx}${parts.behaviorIdx}`;
}

export const BEHAVIOR_CSS_MAP: Record<string, string> = binData.behavior_css || {
