...
const brand_1 = require("./src/identity/brand");
...
`;
function getSlideDescription(slideType) {
    switch (slideType) {
        case "starting_soon":
            return `
...
function getAccentDescription(accent) {
    return `Primary color profile: ${accent.replace("_", " ")}. Hex: ${brand_1.ACCENT_MAP[accent] || "#00ffff"}`;
}
function getMotionDescription(profile) {
    const settings = (0, engine_1.getMotionSettings)(profile);
    switch (profile) {
...
function buildCorruptionLayer(options) {
